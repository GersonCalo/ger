...
- `POST /insights/suggest-category` (body `{ note }`) — **solo premium**. Sugiere una categoría existente del usuario a partir de la nota del movimiento (coincidencia con nombres de categoría y diccionario de comercios/conceptos en `suggestCategory`, `src/domain/insights`). Devuelve `{ suggestion: { categoryId, name } | null }`; nunca inventa categorías. Pendiente de integrar en el formulario de movimientos del frontend.
